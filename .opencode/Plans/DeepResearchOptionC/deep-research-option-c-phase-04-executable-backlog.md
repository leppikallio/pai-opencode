...
- `scratch/research-runs/<run_id>/citations/extracted-urls.txt` (or equivalent extracted list)
...
| P04-02 | Implement tool: `deep_research_citations_extract_urls` (writes `scratch/research-runs/<run_id>/citations/extracted-urls.txt` and `citations/found-by.json` or equivalent provenance artifact) | Engineer | Architect | P04-01 | Tool implementation + wiring | `bun test .opencode/tests/entities/deep_research_citations_extract_urls.test.ts` passes |
...
| P04-04 | Implement tool: `deep_research_citations_normalize` (reads extracted URLs, applies normalization + cid rules per `spec-citation-schema-v1.md`, writes `citations/normalized-urls.txt` + deterministic mapping artifact) | Engineer | Architect | P04-03 | Tool implementation + artifacts: `scratch/research-runs/<run_id>/citations/normalized-urls.txt` (and mapping JSON as needed) | `bun test .opencode/tests/entities/deep_research_citations_normalize.test.ts` passes |
...
| P04-09 | Implement deterministic renderer: `deep_research_citations_render_md` (writes `scratch/research-runs/<run_id>/citations/validated-citations.md` from `citations.jsonl`; stable ordering by `normalized_url` or `cid`) | Engineer | Architect | `spec-citation-schema-v1.md` + P04-06 | Tool implementation + artifact `citations/validated-citations.md` | `bun test .opencode/tests/entities/deep_research_citations_render_md.test.ts` passes |
