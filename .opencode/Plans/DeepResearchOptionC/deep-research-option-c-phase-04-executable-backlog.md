...
- OFFLINE deterministic mode (tests + fixtures + dry-run):
  - Activated by no-web mode (`--sensitivity no_web` / settings flag `deepResearch.flags.PAI_DR_NO_WEB=true`).
  - URL “validation” must be fixture-driven and deterministic; no network calls.
- ONLINE validation mode (optional runtime behavior):
  - Activated only when no-web mode is disabled (e.g., `--sensitivity normal` and settings flag `deepResearch.flags.PAI_DR_NO_WEB=false`).
  - Thoroughness may vary by settings flag `deepResearch.flags.PAI_DR_CITATION_VALIDATION_TIER` (`basic|standard|thorough`).
...
| P04-ON1 | Add **optional ONLINE validation smoke** (skipped by default; runs only when no-web is disabled; validates that tool can fetch and populate fields without breaking schema). ONLINE smoke must run only in sandboxed environment (no access to internal networks); document prerequisites. | Engineer | QATester | P04-06 | Optional test file guarded by settings checks + documented in spec | manual-check: running the canary test suite with `deepResearch.flags.PAI_DR_NO_WEB=false` performs live checks in sandbox; default CI remains OFFLINE |
