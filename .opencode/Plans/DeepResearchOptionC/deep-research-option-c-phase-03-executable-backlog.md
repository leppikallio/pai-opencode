...
| P03-02 | Implement tool: `deep_research_perspectives_write` (writes + validates `~/.config/opencode/research-runs/<run_id>/perspectives.json`) | Engineer | Architect | P03-01 | Tool implementation + wiring per spec | `bun test .opencode/tests/entities/deep_research_perspectives_write.test.ts` passes |
