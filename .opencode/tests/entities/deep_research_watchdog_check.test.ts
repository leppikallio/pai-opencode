  ...
  test("times out when no heartbeat progress exists", async () => {
    await withEnv({ PAI_DR_OPTION_C_ENABLED: "1" }, async () => {
      await withTempDir(async (base) => {
        ...
        expect((out as any).timer_origin_field).toBe("stage.started_at");
  ...
  test("keeps stage alive when heartbeat progress is recent", async () => {
    await withEnv({ PAI_DR_OPTION_C_ENABLED: "1" }, async () => {
      await withTempDir(async (base) => {
        ...
        const initRaw = (await (run_init as any).execute(
          { query: "Q", mode: "deep", sensitivity: "normal", run_id: runId, root_override: base },
          makeToolContext(),
        )) as string;
        const init = parseToolJson(initRaw);
        expect(init.ok).toBe(true);
        ...
        const seeded = JSON.parse(await fs.readFile(manifestPath, "utf8"));
        seeded.stage.current = "wave1";
        seeded.stage.started_at = "2026-02-14T11:00:00.000Z";
        seeded.stage.last_progress_at = "2026-02-14T11:58:30.000Z";
        await fs.writeFile(manifestPath, `${JSON.stringify(seeded, null, 2)}\n`, "utf8");
        ...
        const outRaw = (await (watchdog_check as any).execute(
          {
            manifest_path: manifestPath,
            now_iso: "2026-02-14T12:00:00.000Z",
            reason: "test: heartbeat watchdog",
          },
          makeToolContext(),
        )) as string;
        ...
        expect((out as any).stage).toBe("wave1");
        expect((out as any).elapsed_s).toBe(90);
        expect((out as any).timeout_s).toBe(600);
        expect((out as any).timer_origin_field).toBe("stage.last_progress_at");
