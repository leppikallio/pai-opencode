...
describe("deep_research operator CLI synthesis task driver (entity)", () => {
  test("synthesis stage prompt-outs and halts until agent-result ingestion", async () => {
    await withTempDir(async (base) => {
      const runId = "dr_test_cli_synthesis_task_driver_001";
      ...
      manifestRaw.stage = { ...(manifestRaw.stage ?? {}), current: "synthesis" };
      ...
      const tickRes = await runCli([
        ...
        manifestPath,
        "--reason",
        "test synthesis task driver prompt-out",
        "--driver",
        "task",
      ]);
      ...
      expect(nextCommands.some((item) => String(item).includes("agent-result") && String(item).includes("--stage synthesis") && String(item).includes("--perspective \"final-synthesis\""))).toBe(true);
    });
  });
});
