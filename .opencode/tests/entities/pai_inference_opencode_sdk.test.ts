...
async function writeJson(res: ServerResponse, status: number, payload: unknown): Promise<void> {
...
describe("PAI inference OpenCode SDK carrier", () => {
  ...
  test("creates session, prompts, and deletes with SDK carrier", async () => {
    ...
    const server = createServer(async (req, res) => {
      ...
      const directoryValue = Array.isArray(directoryHeader) ? directoryHeader[0] : directoryHeader;
    ...
    try {
      ...
      const createRequest = requests.find((r) => r.method === "POST" && r.path === "/session");
      expect(createRequest).toBeDefined();
      const createBody = JSON.parse(createRequest!.bodyText) as {
        permission?: Array<{ permission?: string; pattern?: string; action?: string }>;
      };
      expect(createBody.permission).toEqual([{ permission: "*", pattern: "*", action: "deny" }]);

      const promptRequest = requests.find((r) => r.method === "POST" && r.path === "/session/S1/message");
