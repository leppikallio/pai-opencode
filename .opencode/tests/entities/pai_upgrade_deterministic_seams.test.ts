    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({
          ...
          rating: 4,
          source: "explicit",
          sentiment_summary: "Need deterministic seams for upgrade monitoring and reflection mining",
        })}\n`,
        "utf-8",
      );
      ...
      const result = await runMonitor({
        ...
        runtime: {
          fetch: async (input) => {
            ...
            return new Response("<html><h1>Upgrade Monitor</h1><title>Deterministic Seams</title></html>", { status: 200 });
          },
      ...
      const state = JSON.parse(readFileSync(stateFilePath, "utf-8")) as { last_check_timestamp: string };
      expect(state.last_check_timestamp).toBe(fixedNow.toISOString());

      expect(result.summary.ranking.historyPath).toBe(path.resolve(recommendationHistoryPath));
    } finally {
