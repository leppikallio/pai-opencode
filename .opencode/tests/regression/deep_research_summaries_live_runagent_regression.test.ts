...
describe("deep_research summaries live runAgent seam regression", () => {
  test("live driver writes missing summaries via runAgent", async () => {
    await withEnv({ PAI_DR_CLI_ENABLED: "1", PAI_DR_CLI_NO_WEB: "1" }, async () => {
      await withTempDir(async (base) => {
        const runId = `dr_summaries_live_runagent_${Date.now()}`;
        ...
        const tick = await orchestrator_tick_post_summaries({
          ...
          reason: "regression: summaries live runAgent",
          driver: "live",
          drivers: {
            runAgent: async (input: { perspective_id: string }) => {
              const perspectiveId = input.perspective_id;
              liveCalls.push(perspectiveId);
              const cid = perspectiveId === "p1" ? "cid_a" : "cid_b";
              return {
                markdown: [
                  "## Findings",
                  `- LIVE summary for ${perspectiveId} [@${cid}]`,
                  "",
                  "## Evidence",
                  `- Evidence for ${perspectiveId} [@${cid}]`,
                  "",
                ].join("\n"),
              };
            },
        ...
        if (!tick.ok) return;
        expect(tick.from).toBe("summaries");
        expect(tick.to).toBe("synthesis");
        expect(liveCalls.sort()).toEqual(["p1", "p2"]);

        const p1Summary = await fs.readFile(path.join(runRoot, "summaries", "p1.md"), "utf8");
        const p2Summary = await fs.readFile(path.join(runRoot, "summaries", "p2.md"), "utf8");
        expect(p1Summary).toContain("LIVE summary for p1");
        expect(p2Summary).toContain("LIVE summary for p2");
      });
    });
  });
});
