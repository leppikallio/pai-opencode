...
- `--canonicalize <mode>`: `none` | `minimal` | `strict`
  - `none`: copy only
  - `minimal` (default): normalize `SKILL.md` description to single-line + ensure `USE WHEN`; normalize `workflows/` and `tools/` dir casing; rewrite `${PAI_DIR}`/`$PAI_DIR` to `~/.config/opencode`; rename `CLAUDE.md` to `REFERENCE.md` and update links
