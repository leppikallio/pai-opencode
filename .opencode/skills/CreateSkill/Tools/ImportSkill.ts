...
Options:
  --name <SkillName>            Override destination skill name (TitleCase)
...
function isTitleCaseSkillName(name: string): boolean {
  // TitleCase/PascalCase: starts uppercase; alnum only.
...
function normalizeDescriptionYaml(yaml: string, skillName: string): { yaml: string; changed: boolean } {
  ...
  if (!lines.some((l) => /^description:\s*/.test(l))) {
...
async function main(): Promise<void> {
  ...
  if (!isTitleCaseSkillName(skillName)) {
    die(`Skill name must be TitleCase (e.g., CitationCreation). Got: ${skillName}`);
  }
