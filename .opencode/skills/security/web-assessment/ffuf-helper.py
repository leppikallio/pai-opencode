...
def analyze_results(json_file):
    ...
    if interesting_codes:
        print(
            f"    - Found {len(interesting_codes)} results with interesting status codes"
        )
    ...
    anomalous_sizes = [
        r for r in results if abs(r["length"] - avg_size) > size_threshold
    ]
    if anomalous_sizes:
    ...
    interesting_urls = [
        r for r in results if any(kw in r["url"].lower() for kw in interesting_keywords)
    ]
    if interesting_urls:
    ...
    if results and "duration" in results[0]:
        ...
        if slow_requests:
            print(
                f"    - Found {len(slow_requests)} slow responses (potential SQLi, etc.)"
            )
            for r in sorted(slow_requests, key=lambda x: x["duration"], reverse=True)[
                :5
            ]:
...
Examples:
  ...
  %(prog)s wordlist -o ids.txt -t numbers -s 1 -e 10000
    ...
    if args.command == "analyze":
        ...
        create_request_file(args.output, args.method, args.url, args.headers, args.data)
