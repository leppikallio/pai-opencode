...
const DEFAULT_PAI_DIR = envPaiDir ? envPaiDir : "/Users/zuul/.config/opencode";
...
const DEFAULT_SECTIONS_DIR = path.join(
  DEFAULT_PAI_DIR,
  "skills",
  "PAI",
  "SYSTEM",
...
function normalizeRuntimePathToken(raw: string): string {
  const trimmed = raw.trim();
  if (!trimmed) return trimmed;

  const withoutFileScheme = trimmed.replace(/^file:\/\//, "");

  // ~ shorthand for the default runtime root.
  const tildePrefix = "~/.config/opencode/";
  if (withoutFileScheme.startsWith(tildePrefix)) {
    return path.join(DEFAULT_PAI_DIR, withoutFileScheme.slice(tildePrefix.length));
  }

  // /Users/<name>/.config/opencode/... -> ${DEFAULT_PAI_DIR}/...
  const userPrefixMatch = withoutFileScheme.match(/^\/Users\/[^/]+\/\.config\/opencode\/(.+)$/);
  if (userPrefixMatch?.[1]) {
    return path.join(DEFAULT_PAI_DIR, userPrefixMatch[1]);
  }
...
function parseRoutingTable(indexMd: string): {
  ...
  for (; i < lines.length; i++) {
    ...
    const docPath = normalizeRuntimePathToken(docMatch[1]);
...
function extractBacklinkFields(sectionMd: string): {
  ...
  return {
    upRuntime: up ? normalizeRuntimePathToken(up) : undefined,
    sourceRepo: src,
    scope,
  };
}
