...
type InferenceDeps = {
	createServerConnection?: (
		opts: Parameters<typeof createServerConnection>[0],
	) => Promise<{
		client: unknown;
...
function parseProviderModel(model: string | undefined): {
	providerID: string;
	modelID: string;
} {
...
function extractAssistantTextFromOpenCode(
	resp: OpenCodePromptResponse,
): string {
...
async function tryOpenCodeCarrierWithTimeout(
	...
	const explicitServerUrl =
		options.serverUrl ?? process.env.OPENCODE_SERVER_URL ?? null;
	...
	const createServerConnectionFn = deps?.createServerConnection;
	...
	try {
		const conn = await withAbortableTimeout(
			async (signal) => {
				...
				if (createServerConnectionFn) {
					return await createServerConnectionFn(connectOpts);
				}

				return (await createServerConnection(connectOpts)) as unknown as {
					client: unknown;
					cleanup: () => Promise<void>;
				};
			},
		...
		client = conn.client as unknown as OpenCodeClientLike;
	} catch (err: unknown) {
	...
	try {
		...
		const requestId =
			promptResult.response?.headers.get("x-request-id") || undefined;
		...
		if (sessionId) {
			...
			try {
				await withAbortableTimeout(
					(signal) =>
						client.session.delete({ sessionID: sessionId }, { signal }),
					deleteTimeout,
	...
	return command({
		...
		args: {
			...
			model: option({
				...
				description:
					"Override model provider/model (default: level preset model).",
			}),
			serverUrl: option({
				long: "server-url",
				type: optional(string),
			...
			reuseVerifiedLoopback: flag({
				...
				type: boolean,
				description:
					"Auto mode: reuse verified loopback server on port 4096 (default: off).",
			}),
			trustServer: flag({
				long: "trust-server",
				type: boolean,
				description:
					"Allow auth probing/attach when server requests auth (default: off).",
			}),
			allowNonLoopbackAuth: flag({
				long: "allow-non-loopback-auth",
				type: boolean,
				description:
					"Allow auth to non-loopback explicit server URLs (default: off).",
			}),
			...
			directory: option({
				...
				type: optional(string),
				description:
					"Override OpenCode directory header value (default: policy-derived).",
			}),
			probeTimeoutMs: option({
				long: "probe-timeout-ms",
				type: optional(number),
				description:
					"Probe timeout in ms (default: 750 loopback, 2000 non-loopback).",
			}),
...
function writeJsonErrorToStdout(
	message: string,
	code: "usage" | "runtime" | "abort",
): void {
...
async function main(argv = process.argv.slice(2)): Promise<number> {
	...
	let parsed: CmdTsResult<InferenceCliArgs>;

	try {
		parsed = (await runSafely(
			cli,
			argv,
		)) as unknown as CmdTsResult<InferenceCliArgs>;
	} catch (error: unknown) {
	...
	try {
		...
		if (args.json) {
			if (!isJsonObject(result.parsed)) {
				writeJsonErrorToStdout(
					"Inference JSON mode requires an object response",
					"runtime",
				);
				return 1;
			}
