...
```bash
bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts <command>
```
...
```bash
bun run ~/.config/opencode/skills/PAI/Tools/FeatureRegistry.ts <command>
```
...
Manual check: `bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts list`

### Step 4: Review Last Session State (if resuming)
```bash
bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts resume <project>
```
...
bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts create my-feature \
  "Implement user authentication" \
  "Add OAuth providers"

# 2. Initialize feature registry (if multiple features)
bun run ~/.config/opencode/skills/PAI/Tools/FeatureRegistry.ts init my-feature
bun run ~/.config/opencode/skills/PAI/Tools/FeatureRegistry.ts add my-feature "Login form" --priority P1
bun run ~/.config/opencode/skills/PAI/Tools/FeatureRegistry.ts add my-feature "Password reset" --priority P2

# 3. Work on task, recording progress
bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts decision my-feature \
  "Using JWT tokens" "Simpler than sessions for API-first architecture"

bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts work my-feature \
  "Created User model and migration" src/models/user.ts src/migrations/001_users.sql

# 4. Before ending session, set handoff
bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts next my-feature \
  "Write auth service tests" "Implement login endpoint" "Add JWT validation"

bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts handoff my-feature \
...
bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts list

# 2. Load context for specific project
bun run ~/.config/opencode/skills/PAI/Tools/SessionProgress.ts resume my-feature

# 3. Check feature status
bun run ~/.config/opencode/skills/PAI/Tools/FeatureRegistry.ts list my-feature
