...
USER CUSTOMIZATIONS GO IN: `~/.config/opencode/skills/PAI/USER/ARCHITECTURE.md`
-->

**The Founding Principles and Universal Architecture Patterns for Personal AI Infrastructure**

This document defines the foundational architecture that applies to ALL PAI implementations. For user-specific customizations, see `~/.config/opencode/skills/PAI/USER/ARCHITECTURE.md`.
...
```
session.created
  -> experimental.chat.system.transform (inject PAI context)
...
**Full documentation:** `~/.config/opencode/skills/PAI/SYSTEM/MEMORYSYSTEM.md`
...
Route notifications based on event type and priority. User-specific configuration in `~/.config/opencode/skills/PAI/USER/ARCHITECTURE.md`.
...
|-----------|----------|------------------|
| `~/.config/opencode/skills/PAI/USER/` | Personal data, finances, health, contacts | RESTRICTED |
...
System-level updates are tracked in `~/.config/opencode/skills/PAI/SYSTEM/UPDATES/` as individual files.
User-specific updates are tracked in `~/.config/opencode/skills/PAI/USER/UPDATES/`.
