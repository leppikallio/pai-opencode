...
- `~/.config/opencode/mcp/research-shell/security-adapter.ts` (session-dir policy events for research-shell)
...
| `plugins/security/audit-log.ts` (called by `plugins/security/index.ts` and `~/.config/opencode/mcp/research-shell/security-adapter.ts`) | tool security checks + research-shell session-dir checks | `~/.config/opencode/MEMORY/SECURITY/YYYY-MM/security.jsonl` |
