...
Create a new skill following canonical structure with **kebab-case skill identity naming**.
...
## Step 3: Determine canonical names (placeholders only)

Skill identity naming uses kebab-case. Workflow/tool/root-doc filename conventions are separate.

| Component | Required format | Placeholder example |
|---|---|---|
| Skill directory | kebab-case | `<skill-name>` |
| Frontmatter `name` | kebab-case (must match skill directory) | `<skill-name>` |
...
Do not use TitleCase for skill directory or frontmatter `name`.
...
```bash
mkdir -p "/Users/zuul/Projects/pai-opencode/.opencode/skills/<skill-name>/Workflows"
mkdir -p "/Users/zuul/Projects/pai-opencode/.opencode/skills/<skill-name>/Tools"
```
...
uv run python "/Users/zuul/Projects/pai-opencode/.opencode/skills/security/skill-security-vetting/Tools/RunSecurityScan.py" \
  --mode single \
