...
**This is a wizard-style installation.** Use OpenCode's native tools to guide the user through installation:

1. **question** - For user decisions and confirmations
...
```bash
PAI_CHECK="${PAI_DIR:-$HOME/.config/opencode}"
...
for backup in "$HOME/.config/opencode.bak" "$HOME/.config/opencode-backup" "$HOME/.config/opencode-old" "$HOME/.pai-backup" "$HOME/.claude.bak" "$HOME/.claude-backup" "$HOME/.claude-old"; do
...
**Use the question tool at each decision point.**
...
- Start server: cd ~/.config/opencode/VoiceServer && bun run server.ts &
...
4. Start server manually: `cd ~/.config/opencode/VoiceServer && bun run server.ts`
