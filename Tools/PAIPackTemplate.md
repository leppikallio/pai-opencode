...
# 4. Check OpenCode config for existing plugins
OPENCODE_CONFIG="$HOME/.config/opencode/opencode.json"
if [ -f "$OPENCODE_CONFIG" ]; then
  echo "OpenCode opencode.json EXISTS"
  if grep -q '"plugin"' "$OPENCODE_CONFIG" 2>/dev/null; then
    echo "⚠️  Existing plugin configuration found"
  else
    echo "✓ No explicit plugin configuration in opencode.json"
  fi
else
  echo "⚠️  No OpenCode opencode.json found"
fi
...
| **Plugins Exist** | OpenCode config has plugins | **MERGE** - add new plugins to existing array |
...
> **FOR AI AGENTS:** Create each of these files exactly as shown. All plugins are required for full functionality.

[Include the complete OpenCode plugin implementation(s) here - with complete code]

---

### Step 4: Register Plugins in OpenCode

OpenCode auto-discovers plugins from `~/.config/opencode/plugins/`.

**Primary install path:** `~/.config/opencode/plugins/`

If you must explicitly register a plugin, OpenCode reads config from:
...
{
  "plugin": [
    "file:///absolute/path/to/your-plugin.ts"
  ]
}
```

**Important:** Prefer plugin auto-discovery (copy the file into `~/.config/opencode/plugins/`) over editing `opencode.json`.
...
The history system triggers automatically on OpenCode plugin events:

| Event | Plugin | Output Location | Captured Data |
|-------|------|-----------------|---------------|
...
**The test:** Can someone go from fresh OpenCode to fully working system using ONLY this pack directory?
