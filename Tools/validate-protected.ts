 ...
 *   bun Tools/validate-protected.ts --staged  (check only staged files)
 */
...
interface ProtectedManifest {
  version: string;
  protected: Record<string, ProtectedCategory> & {
    protected_patterns?: ProtectedPatterns;
  };
}

// Tools/validate-protected.ts -> <repo>/Tools -> <repo>
const PAI_ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');
...
function checkForbiddenDirectories(stagedFiles: string[], manifest: ProtectedManifest): string[] {
  ...
  const forbiddenDirs = manifest.protected.forbidden_directories as ProtectedCategory | undefined;
...
function getAllProtectedFiles(manifest: ProtectedManifest): string[] {
  ...
  for (const [name, category] of Object.entries(manifest.protected)) {
    if (name === 'protected_patterns') continue;
    const cat = category as ProtectedCategory | undefined;
    if (cat?.files) files.push(...cat.files);
  }
...
function checkFileContent(filePath: string, manifest: ProtectedManifest): {
  ...
  if (patternCategory?.categories && !isException) {
    const allPatterns = getAllPatterns(patternCategory);
  ...
  for (const [categoryName, category] of Object.entries(manifest.protected)) {
    if (categoryName === 'protected_patterns') continue;
    const cat = category as ProtectedCategory | undefined;
    if (!cat?.files?.includes(filePath) || !cat.validation) {
      continue;
    }

    // Core documents must reference PAI
    if (cat.validation.includes('PAI')) {
    ...
    if (cat.validation.includes('private Kai data') && !isException) {
    ...
    if (cat.validation.includes('secrets') && !isException) {
