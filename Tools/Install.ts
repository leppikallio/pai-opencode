...
function renderGlobalAgentsManagedBlock(args: {
  ...
  return [
    ...
    "## PAI Router (Managed)",
    "",
    "Mode selection: MINIMAL / NATIVE / ALGORITHM.",
    "One response = one mode. Do not mix modes in a single response.",
    `- ALGORITHM first action: \`Read ${targetDir}/skills/PAI/Components/Algorithm/LATEST\` if present, otherwise \`Read ${targetDir}/skills/PAI/Components/Algorithm/v3.7.0.md\`.`,
    `- Context routing: \`Read ${targetDir}/skills/PAI/CONTEXT_ROUTING.md\`.`,
    `- Do not load \`${targetDir}/skills/PAI/SKILL.md\` unless needed.`,
    "",
