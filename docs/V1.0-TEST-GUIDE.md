...
# 2. Installation Wizard ausführen
...
## Test 1: Context Injection (PAI Loading)

**Was getestet wird:** LoadContext Hook - PAI Skill wird beim Start injiziert
...
### Verifikation:
```bash
cat ~/.config/opencode/plugins/debug.log | grep "Context injected"
...
### Verifikation:
```bash
cat ~/.config/opencode/plugins/debug.log | grep "BLOCKED"
...
ls "$PAI_DIR/MEMORY/WORK/2026-01/"
...
```bash
PAI_DIR=${PAI_DIR:-$HOME/.config/opencode}
cat "$PAI_DIR/MEMORY/WORK/2026-01"/*/META.yaml
...
```bash
PAI_DIR=${PAI_DIR:-$HOME/.config/opencode}
cat "$PAI_DIR/MEMORY/LEARNING/SIGNALS/ratings.jsonl"
...
```bash
PAI_DIR=${PAI_DIR:-$HOME/.config/opencode}
ls "$PAI_DIR/MEMORY/LEARNING/RESPONSE/2026-01/"
...
```bash
PAI_DIR=${PAI_DIR:-$HOME/.config/opencode}
cat "$PAI_DIR/MEMORY/RESEARCH/2026-01/AGENT-"*.md | head -20
...
```bash
PAI_DIR=${PAI_DIR:-$HOME/.config/opencode}
ls "$PAI_DIR/MEMORY/LEARNING"/*/2026-01/
# Könnte Learning-Files zeigen (je nach Session-Inhalt)
```

### Verifikation:
```bash
cat ~/.config/opencode/plugins/debug.log | grep "Learning"
# Sollte zeigen: "Extracted N learnings from work session"
```

### Fallback (manual, hook-free)

If automatic extraction doesn’t run reliably, use the manual tool:

```bash
PAI_DIR=${PAI_DIR:-$HOME/.config/opencode}
bun run "$PAI_DIR/skills/PAI/Tools/ExtractSessionLearnings.ts" --latest --persist
```

Docs: `docs/tooling/ExtractSessionLearnings.md`
...
### Verifikation:
```bash
cat ~/.config/opencode/plugins/debug.log | grep "Skill restore"
...
ls ~/.config/opencode/plugins/pai-unified.ts

# Prüfe OpenCode Config
cat ~/.config/opencode/settings.json | grep plugins
```

### Keine Log-Einträge
```bash
# Prüfe Permissions
ls -la ~/.config/opencode/plugins/debug.log

# Erstelle manuell
touch ~/.config/opencode/plugins/debug.log
chmod 666 ~/.config/opencode/plugins/debug.log
```
...
# Erstelle manuell
PAI_DIR=${PAI_DIR:-$HOME/.config/opencode}
mkdir -p "$PAI_DIR/MEMORY"/{LEARNING/{ALGORITHM,CODE,GENERAL,RESPONSE,SIGNALS,SYSTEM},RESEARCH,STATE,WORK}
```

### Agent hängt / Tool-Ausführung "stuck"

Symptom: OpenCode wirkt eingefroren (keine weiteren Ausgaben), oft nach einem Tool-Call.

1) Debug-Log live beobachten:

```bash
tail -f ~/.config/opencode/plugins/debug.log
```

2) Diagnose anhand des letzten Log-Eintrags:

- **Letzter Eintrag ist** `Tool before: <tool>` **und danach kommt nichts**
  - Ursache ist typischerweise ein Block in `tool.execute.before` (z.B. History Capture Queue).
  - Abhilfe (schnell): starte OpenCode mit Capture-Serialization aus:

  ```bash
  PAI_SERIALIZE_EVENTS=0 opencode
  ```

  - Abhilfe (Debug): aktiviere Debug + Toasts (falls der Client TUI-Toast unterstützt):

  ```bash
  PAI_DEBUG=1 PAI_HISTORY_CAPTURE_TOASTS=1 opencode
  ```

- **Es gibt** `Security check passed for <tool>` **aber kein** `Tool after: <tool>`
  - Dann hängt die Tool-Ausführung selbst (nicht der Pre-Hook). Prüfe das Tool separat.

3) Fail-open Hinweise

Wenn Fail-open aktiv ist, siehst du Warnungen im Debug-Log wie:

- `HistoryCapture fail-open: timeout label=...`
- `HistoryCapture serial queue wait timed out ...`

Diese Warnungen bedeuten: Capture wurde abgebrochen (best-effort), damit der Agent weiterläuft.
